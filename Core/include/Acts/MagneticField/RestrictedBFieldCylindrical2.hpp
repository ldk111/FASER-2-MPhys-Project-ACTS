...
class RestrictedBFieldCylindrical2 final : public MagneticFieldProvider {
 public:
  struct Cache {
  ...
  RestrictedBFieldCylindrical2( const Vector3& field, const std::array<double,3>& trans_, const std::array<double,3>& bound_)
      :  m_BField(field), m_trans(trans_), m_bound(bound_)  {}
    ...
    // std::shared_ptr<TrackingVolume> m_magVolume = TrackingVolume::create(Transform3(Translation3{m_trans[0], m_trans[1], m_trans[2]}),
    //       std::make_shared<const CylinderVolumeBounds>(0.0, 1500., 500.));
    std::shared_ptr<TrackingVolume> m_magVolume = TrackingVolume::create(Transform3(Translation3{20500., 0., 0.}),
          std::make_shared<const CylinderVolumeBounds>(0.0, 1500., 500.));
 ...
 private:
  ...
  std::array<double, 3> m_trans;
  std::array<double, 3> m_bound;
};
}  // namespace Acts
