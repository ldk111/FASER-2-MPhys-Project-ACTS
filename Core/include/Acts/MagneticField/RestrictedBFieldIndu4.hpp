...
class RestrictedBFieldIndu4 final : public MagneticFieldProvider {
 public:
  struct Cache {
  ...
  RestrictedBFieldIndu4( const Vector3& field)
      :  m_BField(field) {}
...
//Acts::RotationMatrix3 rotation << 0, 0, -1, 0, 1, 0,1, 0, 0;
  ...
  Acts::Translation3 translation_ = Acts::Translation3(14250., 0., 0.);
  ...
  std::shared_ptr<TrackingVolume> m_magVolume = TrackingVolume::create(Transform3(translation_*AngleAxis3(3.14*0.5, Vector3(0., 1., 0.))),std::make_shared<const CylinderVolumeBounds>(0.0, 990., 2750.));
